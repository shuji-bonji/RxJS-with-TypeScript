---
description: reduceはストリームのすべての値を累積し、完了時に最終結果のみを出力するRxJS変換オペレーターです。数値の合計・平均・最大値・最小値の計算、オブジェクトの集約、配列の構築など最終的な集計結果だけが必要な場面に最適です。scanと異なり中間結果は出力せず、ストリーム完了が必須のため無限ストリームでは使用できません。
---
