---
description: windowToggleは開始と終了のトリガーを別々のObservableで制御し、複数のウィンドウ期間を独立して管理できる高度なRxJS変換オペレーターです。営業時間中のデータ収集やボタン押下中のイベント記録など、動的な期間管理が必要な場面に最適です。TypeScriptの型推論により型安全なウィンドウ分割処理を実現します。
---

# windowToggle - 開始/終了制御ウィンドウ
