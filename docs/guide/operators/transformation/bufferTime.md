---
description: "bufferTimeオペレーターは一定時間ごとに発行された値をまとめて配列で出力します。リアルタイムログのバッチ送信、UIイベントの集約、ネットワーク効率化など時間単位でのバッチ処理に最適です。bufferとの違い、TypeScriptでの型安全な実装を解説します。"
---

# bufferTime - 時間でまとめて出力
