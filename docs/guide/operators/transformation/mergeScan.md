---
description: mergeScanは非同期の累積処理を行うRxJS変換オペレーターで、scanとmergeMapを組み合わせた動作をします。APIレスポンスの累積集計、前の結果に基づく次のリクエスト実行、ページネーションでの複数ページデータ累積取得など、非同期処理を伴う累積が必要な場面に最適です。concurrent パラメータで同時実行数の制御も可能です。
---
