---
description: findは条件を満たす最初の値を見つけて出力し、即座にストリームを完了させるRxJSフィルタリングオペレーターです。ユーザー検索、在庫確認、エラーログの検出など、配列やリストから特定の要素を探したい場面に最適です。値が見つからない場合はundefinedを出力し、TypeScriptでは返り値がT | undefined型になります。
---
