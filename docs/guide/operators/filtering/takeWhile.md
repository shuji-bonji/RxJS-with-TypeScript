---
description: "takeWhileは指定した条件を満たす間は値を取得し続け、条件がfalseになった時点でストリームを完了させるRxJSフィルタリングオペレーターです。閾値までのデータ取得、優先度に基づく処理、ページングなど動的な条件でストリームを制御したい場面に最適です。inclusiveオプションで条件がfalseになった値も含められます。"
---
