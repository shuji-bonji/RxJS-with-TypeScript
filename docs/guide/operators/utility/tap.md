---
description: tapオペレーターは、ストリームの値に影響を与えずに副作用を実行できるユーティリティオペレーターです。ログ出力によるデバッグ、ローディング状態の制御、分析トラッキング、エラー監視など、ストリームを観察しながら外部処理を行う用途に最適です。TypeScriptの型安全性を保ちながら宣言的なコードで副作用を管理できます。
---
