---
description: RxJSのマルチキャスティングの仕組みを詳しく解説します。Subjectを使った基本パターン、shareとshareReplayオペレーターの使い分け、APIリクエストの重複呼び出し回避、キャッシュ戦略、アプリケーション状態の共有など実践的な設計パターンをTypeScriptコード例とともに紹介します。
---
