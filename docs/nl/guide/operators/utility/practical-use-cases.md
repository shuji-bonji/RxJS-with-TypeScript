...
function simulateFailRequest() {
  return throwError(() => new Error('API-verzoek mislukt')).pipe(
    ...
    catchError((error: unknown) => {
      ...
      errorElement.textContent = `Fout: ${(error instanceof Error ? error.message : String(error))}`;
...
fromEvent(form, 'submit').pipe(
  ...
  // Foutafhandeling
  catchError((error: unknown) => {
    formResult.textContent = `Fout: ${(error instanceof Error ? error.message : String(error))}`;
