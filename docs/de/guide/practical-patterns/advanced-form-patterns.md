...
patches$.subscribe(patches => {
  ...
  // Beispiel: [{ op: "replace", path: "/profile/name", value: "Hans Müller" }]
});
...
formData$.pipe(
  ...
  catchError((error: unknown) => {
    ...
    updateStatus(`❌ Speichern fehlgeschlagen: ${(error instanceof Error ? error.message : String(error))}`, '#f44336');
