---
description: "Der sampleTime-Operator ist ein RxJS-Filteroperator, der periodisch die neuesten Werte des Streams in bestimmten Zeitintervallen abfragt. Er eignet sich ideal für die Erstellung regelmäßiger Schnappschüsse."
---

# sampleTime - holt periodisch den letzten Wert

Der "sampleTime"-Operator **ermittelt** periodisch in **angegebenen Zeitintervallen** den neuesten Wert des Quell-Observable und gibt ihn aus.
...
### 2. warten Sie, bis der erste Abtastzeitpunkt erreicht ist.

Die `sampleTime` wird nichts ausgeben, bis die angegebene Zeit verstrichen ist.
...
`sample` verwendet eine andere Observable als Auslöser, während `sampleTime` ein festes Zeitintervall verwendet.
...
- **[sample](https://rxjs.dev/api/operators/sample)** - Sampling eines anderen Observable als Trigger (offizielle Dokumentation).
...
Der Operator `sampleTime` ermittelt periodisch den letzten Wert im angegebenen Zeitintervall.
