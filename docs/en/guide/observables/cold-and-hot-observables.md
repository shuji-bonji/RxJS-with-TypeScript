...
> `shareReplay()` continues to hold the cache even when subscriptions go to 0, which may cause memory leaks. See [Chapter 10: Misuse of shareReplay](/en/guide/anti-patterns/common-mistakes#4-sharereplay-misuse) for more information.

### About multicast()

> [!NOTE]
> `multicast()` is flexible, but was deprecated in RxJS v7 and removed in v8. Use `share()` or `shareReplay()` now. See [share() operator description](/en/guide/operators/multicasting/share) for more information.
...
- **[Misuse of shareReplay](/en/guide/anti-patterns/common-mistakes#4-sharereplay-misuse)** - Common mistakes and solutions
- **[Subject](/en/guide/subjects/what-is-subject)** - Understanding Hot Subjects
