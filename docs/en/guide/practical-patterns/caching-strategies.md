...
class UserService {
  ...
  getUsers(): Observable<User[]> {
    ...
    this.users$ = this.fetchUsersFromAPI().pipe(
      ...
      shareReplay({ bufferSize: 1, refCount: true }), // Cache the last value (refCount releases when all subscriptions unsubscribe)
...
// Basic usage (recommended form: release cache when all subscriptions unsubscribe)
source$.pipe(
  shareReplay({ bufferSize: 1, refCount: true }) // Cache the last value
);
