...
of(1, 2, 3).pipe(
  ...
  catchError((err: unknown) => {
    console.log('Caught by catchError:', (err instanceof Error ? err.message : String(err)));  // ②
...
| **Re-thrown after catchError** | Errors explicitly re-thrown in catchError | `catchError((e: unknown) => throwError(() => e))` |
...
fetchUsers(true).pipe(
  catchError((err: unknown) => {
    console.error('An error has occurred:', (err instanceof Error ? err.message : String(err)));
...
- **[Error Handling Strategies](/pt/guide/error-handling/strategies)** - Comprehensive RxJS error handling strategies
- **[retry and catchError](/pt/guide/error-handling/retry-catch)** - Details on retry and error catching
- **[finalize and complete](/pt/guide/error-handling/finalize)** - Resource release and stream completion processing
- **[Common Mistakes and Solutions](/pt/guide/anti-patterns/common-mistakes#9-error-suppression)** - Error handling anti-patterns
